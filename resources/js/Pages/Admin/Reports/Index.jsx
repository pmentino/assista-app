...
export default function ReportsIndex({ applications, filters: initialFilters, stats }) {
    ...
    const handlePeriodChange = (e) => {
        ...
        switch (selectedPeriod) {
            ...
            case 'this_year':
                ...
                break;
            default:
                // Custom range, do not reset dates automatically
                return;
        }
    ...
    const handleFilterChange = (e) => {
        ...
        if (name === 'start_date' || name === 'end_date') {
            setPeriod('custom'); // Switch dropdown to 'Custom' if user manually picks dates
        }
    };
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div className="p-6 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                            <div>
                                <h3 className="text-lg font-bold text-gray-800">Assistance Disbursement Report</h3>
                                <p className="text-sm text-gray-500">Filter data by date, status, or program type.</p>
                            </div>
                            <a
                                href={exportPdfUrl}
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-medium text-sm rounded-lg shadow transition-colors"
                            >
                        ...
                        <div className="p-6 grid grid-cols-1 md:grid-cols-5 gap-4 bg-white border-b border-gray-100">
                            ...
                            {/* 1. Quick Period Selector */}
                            <div className="col-span-1">
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Period</label>
                                <select
                                    value={period}
                                    onChange={handlePeriodChange}
                                    className="block w-full border-gray-300 bg-gray-50 rounded-md shadow-sm text-sm focus:ring-blue-500 focus:border-blue-500"
                                >
                            ...
                            <div className="col-span-2 grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="date" name="start_date" value={filters.start_date} onChange={handleFilterChange} className="block w-full border-gray-300 rounded-md shadow-sm text-sm focus:ring-blue-500 focus:border-blue-500" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">To</label>
                                    <input type="date" name="end_date" value={filters.end_date} onChange={handleFilterChange} className="block w-full border-gray-300 rounded-md shadow-sm text-sm focus:ring-blue-500 focus:border-blue-500" />
                                </div>
                            </div>
