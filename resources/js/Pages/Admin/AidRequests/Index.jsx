...
export default function Index({ applications, filters: initialFilters = {}, sort_by: initialSortBy, sort_direction: initialSortDirection }) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const currentParams = new URLSearchParams(window.location.search);
            router.get(route('admin.applications.index'), { ...query, sort_by: currentParams.get('sort_by'), sort_direction: currentParams.get('sort_direction') }, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
                                    ...
                                    {appList.length > 0 ? (
                                        appList.map((app) => (
                                            <tr key={app.id} className="hover:bg-blue-50 transition-colors duration-150">
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">#{String(app.id).padStart(5, '0')}</td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <Link href={route('admin.applications.show', app.id)} className="inline-flex items-center px-3 py-1.5 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 shadow-sm hover:bg-gray-50 hover:text-blue-600 transition">
                                                        Review
                                                    </Link>
                                                </td>
                                            </tr>
