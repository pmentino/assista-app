...
export default function NotificationBell() {
    ...
    // Helper: Translate if key exists, else show original
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl overflow-hidden z-50 ring-1 ring-black ring-opacity-5 origin-top-right transform transition-all">
                    <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                        <h3 className="text-sm font-bold text-gray-700">{__('Notifications')}</h3>
                        {unreadCount > 0 && (
                    ...
                    <ul className="max-h-80 overflow-y-auto custom-scrollbar">
                        {notifications.length > 0 ? (
                            notifications.map((notif) => (
                                <li key={notif.id} className={`border-b border-gray-100 hover:bg-gray-50 transition ${!notif.read_at ? 'bg-blue-50/40' : ''}`}>
                                    <Link
                                        ...
                                        <div>
                                            ...
                                            {notif.data.status ? (
                                                <p className="text-sm font-bold text-gray-800">
                                                    {__('Application')} {__(notif.data.status)}
                                                </p>
                                            ) : (
                                                <p className="text-sm font-bold text-gray-800">{notif.data.message}</p>
                                            )}
                                            ...
                                            {notif.data.program ? (
                                                <p className="text-xs text-gray-600 mt-0.5 leading-snug">
                                                    ...
                                                    {/* 2. TRANSLATED Program Name (Bulig sa Bulong) */}
