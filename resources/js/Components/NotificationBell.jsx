...
export default function NotificationBell() {
    ...
    const getActivityRoute = () => {
        ...
        return route('dashboard'); // Applicants go to dashboard history
    };
    ...
    const markAsRead = async (id) => {
        try {
            // We fire this off but don't wait for it, so navigation is instant
                        ...
                        {notifications.length > 0 ? (
                            notifications.map((notif) => (
                                <li key={notif.id} className={`border-b border-gray-100 hover:bg-gray-50 transition ${!notif.read_at ? 'bg-blue-50/40' : ''}`}>
                                    {/* --- FIX: WRAP CONTENT IN LINK USING DATA.LINK --- */}
                                    <Link
                                        href={notif.data.link || '#'} // Uses the backend smart link
